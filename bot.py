...
captcha_manager = CaptchaManager(API_TOKEN)

app = Flask(__name__)

# --- ল্যাঙ্গুয়েজ ডিকশনারি (সব single line, no multiline string) ---
LANGUAGES = {
    'en': {
        'welcome': "👋 Welcome! ℹ️ This bot helps you earn money by doing simple tasks. By using this Bot, you automatically agree to the Terms of Use.👉 https://telegra.ph/FAQ----CRAZY-MONEY-BUX-12-25-2",
        'balance': "💰 Your balance: ${:.4f}",
        'tasks': "👇 Please select a task:",
        'task_desc': "⏳ Review time: 74 min ⏳ 📋 Task: 📱 G account (FAST CHECK) 📄 Description: 🔐 MANDATORY! You must use only the email and password provided by the Telegram bot to register.",
        'start_task': "👉 Press the button to confirm registration or cancel the task:",
        'submitted': "✅ Submitted for review!",
        'referrals': "👥 Referrals: {} 💰 Earned: ${:.4f} 🔗 Link: {}",
        'withdraw': "📤 Choose method:",
        'insufficient': "❌ Insufficient balance!",
        'enter_amount': "🔢 Min $1.50 📤 Enter Amount:",
        'enter_address': "📤 Enter TRX Address:",
        'withdrawn': "✅ Withdrawal submitted!",
        'profile': "👤 <b>{}</b> 💰 <b>Total Balance:</b> \( {:.4f} 📤 <b>Total Withdraw:</b> \){:.4f} 🔒 <b>Account:</b> Active✅",
        ...
        'user_list_format': "🆔 <b>ID:</b> <code>{}</code> 👤 <b>Name:</b> {} {} 💰 <b>Balance:</b> \( {:.4f} 👥 <b>Referrals:</b> {} 📤 <b>Paid Withdraw:</b> \){:.4f}",
        'no_users': "📭 No users yet.",
    },
    'bn': {
        'welcome': "👋 স্বাগতম! ℹ️ এই বটে সিম্পল টাস্ক করে ডলার আর্ন করুন। বট ব্যবহার করে আপনি অটোম্যাটিক টার্মস অ্যাগ্রি করছেন।👉 https://telegra.ph/FAQ----CRAZY-MONEY-BUX-12-25-2",
        'balance': "💰 আপনার ব্যালেন্স: ${:.4f}",
        'tasks': "👇 একটা টাস্ক সিলেক্ট করুন:",
        'task_desc': "⏳ রিভিউ টাইম: ৭৪ মিনিট ⏳ 📋 টাস্ক: 📱 G account (FAST CHECK) 📄 বর্ণনা: 🔐 অবশ্যই বট দেওয়া ইমেইল ও পাসওয়ার্ড দিয়ে রেজিস্টার করতে হবে।",
        'start_task': "👉 রেজিস্ট্রেশন কনফার্ম করুন বা ক্যানসেল করুন:",
        'submitted': "✅ রিভিউয়ের জন্য সাবমিট করা হয়েছে!",
        'referrals': "👥 রেফারেল: {} 💰 আর্ন: ${:.4f} 🔗 লিঙ্ক: {}",
        'withdraw': "📤 পেমেন্ট মেথড সিলেক্ট করুন:",
        'insufficient': "❌ ব্যালেন্স যথেষ্ট নয়!",
        'enter_amount': "🔢 মিনিমাম $1.50 📤 অ্যামাউন্ট দিন:",
        'enter_address': "📤 TRX অ্যাড্রেস দিন:",
        'withdrawn': "✅ উইথড্র রিকোয়েস্ট করা হয়েছে!",
        'profile': "👤 <b>{}</b> 💰 <b>টোটাল ব্যালেন্স:</b> \( {:.4f} 📤 <b>টোটাল উইথড্র:</b> \){:.4f} 🔒 <b>অ্যাকাউন্ট:</b> অ্যাকটিভ✅",
        ...
        'user_list_format': "🆔 <b>ID:</b> <code>{}</code> 👤 <b>নাম:</b> {} {} 💰 <b>ব্যালেন্স:</b> \( {:.4f} 👥 <b>রেফারেল:</b> {} 📤 <b>পেইড উইথড্র:</b> \){:.4f}",
...
def language_handler(message):
    ...
    bot.send_message(messageAAG5z--eYoWDpek1XeoY3eyXtdlsOhI0Et4'ply_markup=language_menu())
...
def handle_all(message):
    ...
    if text == '👤 Profile':
        ...
        faq_msg = "🤔 <b>View help at:</b> 📄 https://telegra.ph/FAQ----CRAZY-MONEY-BUX-12-25-2"
        ...
        for r in rows:
            ...
            try:
                ...
                history_txt += f"📧 {gmail} 📊 Status: {status} {stat_emoji}\n"
            except:
                continue
        ...
        conn.commit()
        conn.close()
        main_msg = f"First name: <code>{fn}</code> Last name: <code>{ln}</code> Password: <code>{p}</code> Email: <code>{e}</code> Recovery email: <code>{rec}</code> ⚠️ IMPORTANT: MANDATORY add this recovery email to account settings after registration!"
        ...
        if res and res[0]:
            ...
            admin_msg = f"🔔 <b>New Task Submission</b> 👤 <b>User ID:</b> <code>{user_id}</code> 👤 <b>Name:</b> {fn_user} 👤 <b>Username:</b> {u_name} 🔰<b>Task Information</b>🔰 📧 <b>Gmail:</b> <code>{gmail}</code> 🔑 <b>Pass:</b> <code>{password}</code> 🔄 <b>Recovery:</b> <code>{recovery}</code>"
        ...
        if text == '📝 Task History':
            ...
            for r in rows:
                try:
                    ...
                    password = parts[3].split(': ')[1]
                    recovery = parts[4].split(': ')[1]
                    hist_msg = f"🔔 <b>New Task Submission</b> 👤 <b>User ID:</b> <code>{uid}</code> 👤 <b>Name:</b> {name} 🔰<b>Task Information</b>🔰 📧 <b>Gmail:</b> <code>{gmail}</code> 🔑 <b>Pass:</b> <code>{password}</code> 🔄 <b>Recovery:</b> <code>{recovery}</code>"
            ...
            for row in rows:
                ...
                msg_text = f"💸 <b>Withdraw Request</b> 👤 <b>User:</b> {firstname} ({uname}) 🆔 <b>ID:</b> <code>{uid}</code> 💰 <b>Amount:</b> ${amount} 🏦 <b>Method:</b> TRX 📫 <b>Address:</b> <code>{address}</code>"
...
def process_withdraw_address(message, amount):
    ...
    try:
        for admin_id in ADMIN_IDS:
            bot.send_message(admin_id, f"🔔 New Withdraw Request from ID: {user_id} Amount: ${amount}")
    except:
        pass
    ...
    if captcha:
        if captcha.solved:
            ...
            if cursor.fetchone() is None:
                cursor.execute("INSERT INTO users (id, first_name, username, language) VALUES (?, ?, ?, ?)", 
                               (user_id, call.from_user.first_name, call.from_user.username, 'en'))
                conn.commit()
            conn.close()
...
print("🤖 Gmail Factory Bot is Running - Final Single Line String Version with CAPTCHA!")
